...
#edgelists不含自身，match是包含自身
    ...
    def choose(self, x, x_pool, edge_index, batch, score, match, old_index):
        ...
        # cluster = torch.tensor([1,1,2,2,1,1,2,2,2,1,0,0,2,3,3,0,0])
        # new_node_indices = [10,5,7,14]
        # new_x =  torch.cat([x[new_node_indices, :]])
        ...
        for idx in range(x.size(0)):
            ...
            for ne in match[idx]:
                if ne != idx:
                    neg.append(idx)
                    anchor_neg.append(ne)
        # pos_pos = x_pool[pos]  #节点本身特征
        # pos_anchor = new_x[anchor_pos]  #位置对应的是节点编号，value对应的是他所归属社团的特征
        # neg_neg = x_pool[neg]
        # neg_anchor = new_x[anchor_neg]

        pos_pos = x_pool[pos]  #节点本身特征
        pos_anchor = x[anchor_pos]  #位置对应的是节点编号，value对应的是他所归属社团的特征
        neg_neg = x_pool[neg]
        neg_anchor = x[anchor_neg]
...
#new_edge_index这里面有问题，不应该是社团之间的索引，我们不会对他融合。
    ...
    def forward(self, x, edge_index, old_index, batch=None):
        ...
        sorted_id = sorted(range(len(fitness)), key=lambda k: fitness[k], reverse=True)
        print(sorted_id)
        x, new_x_pool, cluster, transfer, pos_pos, pos_anchor, neg_neg, neg_anchor = self.choose(
            x, x_pool, edge_index, batch, fitness, match, old_index)  #x 是社团特征向量，pool是节点本身的向量
