...
class RicciCurvaturePooling2(nn.Module):
    ...
    def __init__(self, in_channels: int, ratio: Union[float, int] = 0.01, alpha=0.5, GNN: Optional[Callable] = GCNConv,
        ...
        super(RicciCurvaturePooling2, self).__init__()
        self.alpha = alpha
        self.in_channels = in_channels
        self.ratio = ratio
        self.verbose = "ERROR"
    ...
    def choose(self, x, x_pool, edge_index, batch, score, match):
        ...
        for idx in range(x.size(0)):
            sig[idx] = []
            if cluster[idx].item() in range(len(new_node_indices)):  # 生成正样本,逐个判断每个节点是否在motif中，idx是原图节点序号，cluster[idx]是原图115个节点的聚类序号,(看原图每个节点的聚类序号是否在新图的主要节点序号中)
                pos.append(idx)  # 因为前面是按照得分降序排列的，所以range(len(new_node_indices))就是对应motif的个数
                anchor_pos.append(cluster[idx].item())
                for j in match[idx]:
                    if j != idx and cluster[j] != cluster[idx] and cluster[j].item() not in sig[idx]:
                        neg.append(idx)
                        anchor_neg.append(cluster[j].item())  # 保存idx节点邻居的聚类序号
